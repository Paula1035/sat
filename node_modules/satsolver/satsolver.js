  ...
  while ((!isSat)) {
    ...
    for(let i = 0; i < clauses.length; i++){
        ...
        for(let j = 0; j < clauses.length; j++){
            if (clauses[i][j] < 0){
              aux = !currentAssignment[Math.abs(clauses[i][j]) - 1] 
           }
           else {
              aux = currentAssignment[Math.abs(clauses[i][j]) - 1] 
           }
    ...
    if (isSat == false) { currentAssignment = nextAssignment(currentAssignment)}
  }
  let result = {'isSat': isSat, satisfyingAssignment: null}
  if (isSat) {
    result.satisfyingAssignment = currentAssignment
  }
  return result
}
